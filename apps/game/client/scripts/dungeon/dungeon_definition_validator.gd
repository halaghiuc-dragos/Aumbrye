...
static func validate(definition: Dictionary) -> Dictionary:
	...
	var entrance_id := ""
	for room in rooms:
		if room is Dictionary and str(room.get("type", "")) == "entrance":
			entrance_count += 1
			entrance_id = str(room.get("id", ""))
	var placement_entrance: Variant = placements.get("entrance")
	var placement_entrance_id := ""
	if placement_entrance is Dictionary:
		placement_entrance_id = str(placement_entrance.get("roomId", ""))
	if entrance_count != 1 or placement_entrance_id == "" or entrance_id != placement_entrance_id:
	...
	var exit_placement: Variant = placements.get("exit")
	if exit_placement is Dictionary:
		var exit_id := str(exit_placement.get("roomId", ""))
		if exit_id != "" and not _exit_reachable(definition, entrance_id, exit_id):
...
static func _room_aabb(room: Dictionary) -> Rect2:
	...
	var yaw := float(transform.get("yaw", 0.0))
	var spec := RoomTemplateCatalog.get_spec(str(room.get("templateId", "")))
	var half_x := RoomTemplateCatalog.half_extent_x(spec, yaw)
	var half_z := RoomTemplateCatalog.half_extent_z(spec, yaw)
