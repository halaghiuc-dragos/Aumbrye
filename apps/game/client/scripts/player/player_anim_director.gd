...
func _arbitrate_hit_reaction(info: DamageInfo) -> void:
	...
	var poise_broken := _poise != null and _poise.is_broken()

	if info.poise_damage >= 20.0 or poise_broken:

		play_stagger(DEFAULT_STAGGER, info.direction)

		return

	if info.poise_damage >= 8.0:
...
func _is_frontal_hit(direction: Vector3) -> bool:
	...
	var flat_facing := Vector3(-facing.global_transform.basis.z.x, 0.0, -facing.global_transform.basis.z.z)
...
func _turn_clip_if_needed() -> StringName:
	...
	var body_forward := -facing.global_transform.basis.z
...
func _update_head_look(_delta: float) -> void:
	if _visual == null or _additive_player == null:
		return
	var lock_on := _body.get_node_or_null("LockOn") as LockOn
	if lock_on == null or lock_on.current_target == null:
		return
	var head := CharacterSkin.find_part(_visual, "Head")
	if head == null:
		return
