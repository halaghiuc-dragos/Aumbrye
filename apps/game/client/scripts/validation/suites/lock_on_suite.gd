...
func _test_lock_on_behavior() -> void:
	var shield_scene: PackedScene = EnemyCatalog.get_scene("castle_shield")
	var grunt_scene: PackedScene = EnemyCatalog.get_scene("castle_grunt")
	if shield_scene == null or grunt_scene == null:
		var start := Time.get_ticks_msec()
		ctx.timed_record(
			"lock_on.auto_advance_on_death",
			get_category(),
			false,
			"missing enemy scenes for lock-on test",
			start,
			"M3.lock_on.advance"
		)
		return
	...
	var enemy_a: CharacterBody3D = shield_scene.instantiate() as CharacterBody3D
