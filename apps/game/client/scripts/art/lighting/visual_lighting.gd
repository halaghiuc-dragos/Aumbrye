...
## Optional dust motes, rain, or snow particles plus a thin fog volume per biome.
static func apply_biome_atmosphere(root: Node3D, biome_id: String) -> void:
	if root == null or PixelDioramaSettings.particle_quality <= 0:
		return
	if root.get_node_or_null("BiomeAtmosphere") != null:
		return
	...
	match biome_id:
		BiomeRegistry.BIOME_SWAMP, BiomeRegistry.BIOME_MIRE:
			_add_ambient_particles(holder, Color(0.55, 0.62, 0.42, 0.35), 48, 12.0, 0.4)
		BiomeRegistry.BIOME_FROZEN, BiomeRegistry.BIOME_HOLLOW:
			_add_ambient_particles(holder, Color(0.82, 0.9, 0.98, 0.45), 64, 14.0, 0.55)
		BiomeRegistry.BIOME_CRYSTAL, BiomeRegistry.BIOME_PRISM:
			_add_ambient_particles(holder, Color(0.62, 0.78, 0.95, 0.4), 36, 10.0, 0.25)
		BiomeRegistry.BIOME_UMBRAL, BiomeRegistry.BIOME_CATHEDRAL:
			_add_ambient_particles(holder, Color(0.45, 0.35, 0.62, 0.3), 28, 9.0, 0.18)
		_:
			_add_ambient_particles(holder, Color(0.72, 0.64, 0.48, 0.28), 24, 11.0, 0.15)
	if bool(profile.get("fog_enabled", false)):
		var fog := FogVolume.new()
		fog.name = "BiomeFogVolume"
		fog.size = Vector3(48.0, 8.0, 48.0)
		fog.position = Vector3(0.0, 3.0, 0.0)
		var fog_mat := FogMaterial.new()
		fog_mat.density = float(profile.get("fog_density", 0.02)) * 0.35
		fog_mat.albedo = profile.get("fog_color", Color(0.2, 0.2, 0.25))
