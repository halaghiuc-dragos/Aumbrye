...
static func _build_spear(theme: int) -> Node3D:
	...
	var accent := PixelStyle.make_accent_material(theme)
	PixelStyle.add_box(root, Vector3(0.07, 1.5, 0.07), Vector3(0.0, -0.45, 0.0), shaft, "Shaft")
	PixelStyle.add_box(root, Vector3(0.11, 0.3, 0.05), Vector3(0.0, -1.32, 0.0), steel, "Head")
	PixelStyle.add_box(root, Vector3(0.13, 0.05, 0.09), Vector3(0.0, -1.14, 0.0), accent, "Collar")
	return root
