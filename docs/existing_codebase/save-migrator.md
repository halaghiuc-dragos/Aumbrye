...
| Path | Role |
|------|------|
| `apps/game/client/scripts/save/save_migrator.gd` | `SaveMigrator` â€” version constant, `STEPS`, `classify()`, `plan()`, `describe()`, `migrate()`, five step functions, `_normalize_*` helpers, `_fail()` |
...
| `apps/game/client/scripts/app/world_flags.gd` | Legacy `worldFlags` id mapping during v4â†’v5 |
| `apps/game/client/scripts/dungeon/run_floor_config.gd` | Supplies `MAX_FLOORS` used by the v1â†’v2 step |
...
Five registered steps from v1â†’v6. Each entry has `from`, `to`, `fn`, and `summary`. `plan(from_version)` returns the steps that would apply without mutating input. `describe(from_version)` returns a human-readable summary string.
...
1. `version == CURRENT_VERSION` â†’ returns `data.duplicate(true)` (deep copy).
2. `version > CURRENT_VERSION` â†’ `_fail(..., "too_new", ...)`.
3. `version == 0` â†’ `_fail(..., "missing_version", ...)`.
4. Walks `STEPS`; after each step asserts `schemaVersion == step.to` or returns `_fail(..., "step_error", ...)`.
5. If still not `CURRENT_VERSION` â†’ `_fail(..., "unknown_version", ...)`.

### Migration steps
**v1â†’v2** â€” `activeRun` floor defaults (`currentFloor`, `maxFloors`, `floorDefinitions`).

**v2â†’v3** â€” `activeRun.runMode`; erases `floorDefinitions`.

**v3â†’v4** â€” `activeRun.lastCheckpoint`, `snapshot.worldFlags`, `activeRun.schemaVersion`.

**v4â†’v5** â€” Full-document normalization via `_normalize_character`, `_normalize_currencies`, `_normalize_inventory`, `_normalize_storage`, `_normalize_talents`, `_normalize_flags`, `_normalize_quests`, `_normalize_item_instances`, `_normalize_meta`, `_normalize_active_run`. Resets nil `accountId`, recovers `playerDead` from `lastCheckpoint`, namespaces `worldFlags` through `WorldFlags.migrate_legacy_id`.

**v5â†’v6** â€” Renames `meta.achievements.mythic_loot` to `aumbral_loot` when the legacy key was true (see [`achievements-meta.md`](achievements-meta.md)).
...
**Documentation pointer:** `MIGRATION_DOC` â†’ `docs/SAVE_MIGRATIONS.md`.

## Current state
| Surface | Status | Evidence |
|---------|--------|----------|
| Table-driven v1â†’v6 chain with per-step version assertion | IMPLEMENTED | `save_migrator.gd:26-52`, `migrate()` |
| `classify`, `plan`, `describe` | IMPLEMENTED | `save_migrator.gd` |
| Full-document v4â†’v5 normalization | IMPLEMENTED | `save_migrator.gd` `_normalize_*` |
| v5â†’v6 achievement key rename | IMPLEMENTED | `_migrate_v5_to_v6` |
...
## Related
- Improvement plan: [`../actual_improvements/save-migrator.md`](../actual_improvements/save-migrator.md) - **FINISHED**
