...
`WorldState` is a run-scoped key/value store for dungeon locks, levers, keys, and room content. It is on the live play path â€” dungeon interactables read and write it every run â€” but it is deliberately not persisted to the save file. Persistence happens indirectly: `castle_run.gd` copies `WorldState.all_flags()` into the `activeRun.snapshot.worldFlags` blob.

## Files
| Path | Role |
|------|------|
| `apps/game/client/scripts/app/world_state.gd` | Autoload `WorldState` â€” flag store, signals, lifecycle |
| `apps/game/client/scripts/app/world_flags.gd` | `WorldFlags` registry â€” namespaced id builders and validation |
...
| `has_flag(flag_id)` | `_flags.has(flag_id)` â€” presence, not truthiness |
...
- `run_started` â†’ `_on_run_started()`: returns early when `RunFlow.is_continue_restore()` is true, otherwise calls `reset()` and `InventoryService.clear_dungeon_keys()`.
- `run_ended` â†’ `_on_run_ended()`: calls `reset()`.
- `returned_to_hub` â†’ `_on_returned_to_hub()`: calls `reset()`.
...
- Migration: `save_migrator.gd` v4â†’v5 rewrites legacy `key_*` and `quest_*_active` ids to namespaced `WorldFlags` ids in both `snapshot.worldFlags` and `lastCheckpoint.worldFlags`.

`all_flags()` is a deep duplicate, so nested container values do not alias live state into the snapshot.

### Flag id registry
`WorldFlags` (`world_flags.gd`) defines namespaces `lock`, `lever`, `door`, `room`, `secret`, `chest`, `trap` and builders such as `lock_opened(lock_id)` â†’ `lock.{lock_id}.opened`. Dungeon interactables use these builders exclusively.
...
| v4â†’v5 legacy worldFlags migration | IMPLEMENTED | `save_migrator.gd:79-108` |

## Related
- Improvement plan: [`../actual_improvements/world-state.md`](../actual_improvements/world-state.md) - **FINISHED**
